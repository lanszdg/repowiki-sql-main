...
exports.djangoResolver = {
    ...
    resolve(ref, context) {
        ...
        if (ref.referenceName === '_iterable_class') {
            ...
            if (target)
                return { original: ref, targetNodeId: target, confidence: 0.7, resolvedBy: 'framework' };
        }
        return null;
    },
 ...
 */
function resolveModelIterableIter(context) {
    const cls = context.getNodesByName('ModelIterable').find((n) => n.kind === 'class');
    if (!cls)
        return null;
    const iter = context.getNodesByName('__iter__').find((n) => n.filePath === cls.filePath && n.startLine >= cls.startLine && n.startLine <= cls.endLine);
...
exports.flaskResolver = {
    ...
    detect(context) {
        ...
        for (const f of entrypoints) {
            const c = context.readFile(f);
            if (c && /\bFlask\s*\(/.test(c) && /\bimport\s+flask\b|\bfrom\s+flask\b/.test(c))
                return true;
        }
        return false;
    },
 ...
 */
function extractFlaskRestful(filePath, safe) {
    const nodes = [];
    const references = [];
    const now = Date.now();
