...
try {
  ...
  if (!completeness || Number(completeness.schemaVersion || 0) < CURRENT_COMPLETENESS_SCHEMA_VERSION) {
    throw new Error(`L2 completeness report is stale; rerun merge-knowledge to refresh ${completenessFile}`);
  }
  if (!completeness || completeness.status !== "passed") {
    throw new Error(`L2 completeness failed; fix ${completenessFile} before L3`);
  }
